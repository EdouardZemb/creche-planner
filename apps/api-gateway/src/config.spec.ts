...
describe('verifierConfigProduction (AQ-01)', () => {
  it('refuse le démarrage en production sans jeton ni échappatoire', () => {
    expect(() => verifierConfigProduction({ NODE_ENV: 'production' })).toThrow(
      /GATEWAY_TOKEN requis en production/,
    );
  });
  ...
  it("n'exige rien hors production (dev local, test, NODE_ENV absent)", () => {
    expect(() => verifierConfigProduction({})).not.toThrow();
    expect(() =>
      verifierConfigProduction({ NODE_ENV: 'development' }),
    ).not.toThrow();
    expect(() => verifierConfigProduction({ NODE_ENV: 'test' })).not.toThrow();
  });
