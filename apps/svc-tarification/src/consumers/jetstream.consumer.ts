 ...
 * Au-delà, un message **génuinement orphelin** (ex. `PlanningModifie` dont le
 ...
 * tentatives pour ne pas marteler la base/le client de repli.
 */
const BACKOFF_MS = [1_000, 5_000, 15_000, 30_000];
const BACKOFF_NANOS = BACKOFF_MS.map((ms) => nanos(ms));

/**
 * Consommateur **idempotent** JetStream du read model Tarification (doc 06 §10.4).
 * Au démarrage (et tant que NATS n'est pas prêt), tente de **lier des consommateurs
 * durables** aux streams `FOYER`/`REFERENTIEL`/`PLANIFICATION`. Chaque message est
 * projeté via `ProjectionService` (transaction unique : `processed_event` +
 * upsert). En cas d'erreur transitoire, le message est **NAK** (re-livraison
 * différée) ; sinon **ACK**. La création des consommateurs durables est idempotente
 * (réutilise un consommateur existant). Résilient : réessaie le binding tant qu'il
 * échoue, sans bloquer le boot.
 */
@Injectable()
