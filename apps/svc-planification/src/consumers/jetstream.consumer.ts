...
const ABONNEMENTS: readonly { stream: string; durable: string }[] = [
  ...
  // quand un enfant est renommé (`foyer.EnfantModifie.v1`). Stream déjà publié
  // par svc-foyer (cf. tarification/notifications).
 ...
 * Au-delà, un message génuinement inapplicable cesse de boucler en NAK toutes
 * les ~2 s.
 */
 ...
 * Nanos[]`, cf. `nats@2.x` `jsapi_types`), d'où la conversion via `nanos(ms)`.
 */
const BACKOFF_MS = [1_000, 5_000, 15_000, 30_000];
const BACKOFF_NANOS = BACKOFF_MS.map((ms) => nanos(ms));

/**
 * Consommateur **idempotent** JetStream du stream `FOYER` côté Planification. Au
 * démarrage (et tant que NATS n'est pas prêt), tente de **lier un consommateur
 * durable**. Chaque message est projeté via `ProjectionService` (transaction
 * unique : `processed_event` + rafraîchissement des contrats). En cas d'erreur
