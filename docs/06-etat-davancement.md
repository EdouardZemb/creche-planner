...
> **Phase 12 (accessibilité AA / CT-UT) réalisée le 2026-06-05** — WCAG 2.1 AA tenu et instrumenté (0 violation axe sur les 6 routes, UT-01 onglets clavier livré et vérifié au runtime). **Cible portée à WCAG 2.2 AA le 2026-08-15** (lot 9 des standards, `AM-49`) : les neuf critères ajoutés par 2.2 sont statués un par un en [doc 11 §8](11-spec-accessibilite-ct-ut.md), deux échecs corrigés (SC 2.4.11 et SC 3.3.7), audit axe étendu au tag `wcag22aa` et à la présentation mobile, porte `pnpm wcag`, détail en [§18](#18-phase-12-réalisée--accessibilité-aa--utilisabilité-ct-ut). Reste **purement humain et non bloquant** : écoute lecteur d'écran NVDA/VoiceOver, panel utilisateurs, questionnaire SUS (runbook [doc 13](13-validation-accessibilite-runtime.md)). Détail Phase 11 en [§17](#17-phase-11-réalisée--découplage--maturité-micro-services) ; audits déclencheurs en [§16](#16-audits-dévaluation-2026-06-04--phases-11--12-à-réaliser).
...
| Phase 12 — Accessibilité AA (CT-UT) | ✅ **réalisée** (2026-06-05, voir [§18](#18-phase-12-réalisée--accessibilité-aa--utilisabilité-ct-ut)) — UT-01..10 livrés dont UT-01 (onglets ARIA complets, navigation clavier, roving tabindex), **0 violation axe AA** sur les 6 routes, campagne runbook instrumentée ([doc 13 §9](13-validation-accessibilite-runtime.md)). **Cible portée à WCAG 2.2 AA au lot 9 des standards** (2026-08-15) : 0 violation sur 10 routes desktop et 3 mobiles, tag `wcag22aa` compris. Reste humain non bloquant : NVDA/VoiceOver, panel utilisateurs, SUS | |
...
- **Contrats ↔ enfant** : `CreerContrat.enfant` et `ContratVue.enfant` sont le **prénom** (pas l'id) ; le
  ...
  _(Dépassé depuis §27 : le contrat porte désormais le lien de référence `enfantId`, le prénom n'étant
  plus qu'une dénormalisation d'affichage rafraîchie par projection NATS.)_
...
- **Rollout PRODUCTION (release `0.3.0`, 2026-06-26)** : **première mise en prod de `svc-notifications`** (+ `postgres-notifications` + exporter), en même temps que la feature _édition hebdo_ (§23.bis) et les correctifs #69/#70/#71. La release `0.2.0` (commit `0e23891`, 24/06) étant **antérieure à toute la feature notifications**, le périmètre livré = `0e23891..main` (18 commits, #55→#71). **Prérequis hors-PR exécutés avant le déploiement** : (1) `PG_NOTIFICATIONS_PWD` (absent de `.env.server.enc`) **généré** (`openssl rand -hex 24`, BDD prod neuve) et ajouté via `sops` côté serveur — plaintext en `/dev/shm` nommé `.env.server` pour matcher la `creation_rule` (`--age` seul ne suffit pas en sops 3.9.4 : « no matching creation rules ») — puis le `.enc` re-chiffré versionné via **PR #72** (la deploy key du serveur est _read-only_ → commit/push depuis le poste de dev) ; (2) le staging portait déjà `svc-notifications` dans son `DEPLOY_UP_SERVICES` (service vert). **Release train** : `pnpm nx release 0.3.0 --skip-publish` (les 7 projets sont `private` → pas de cible `nx-release-publish`), commit `7d7ce04`, **7 tags** `<projet>@0.3.0` poussés **par lots ≤3** → `release.yml` ×7 verts (images signées cosign + GitHub Releases). **Déploiement** : `remote-deploy.ps1 -ImageTag 0.3.0` → **GitHub Deployment #5206840092 `production` `success`** (ref `7d7ce04`) ; `svc-notifications` démarré sain (migrations Drizzle, NATS, seed établissements, `:3006`), 0 alerte. **Post-déploiement obs** : `deploy.mjs` applique les images mais **PAS** la config Prometheus (bind-mount, non rechargé au `up`) → `remote-apply-observability.ps1 -Services 'prometheus'` (Deployment `observability` #5206921746) pour activer la sonde blackbox `svc-notifications:3006` (`probe_success` 5→6) + le scrape `postgres-exporter-notifications:9187`. **`NOTIF_EMAIL_DRY_RUN=true` (défaut) reste actif en prod → aucun e-mail réel encore envoyé** : la bascule (`=false` + `NOTIF_EMAIL_ALLOWLIST`, puis recréation `--no-deps --force-recreate`) sera faite sciemment lors du rodage des envois.
- **Bascule ENVOI RÉEL en prod (2026-06-30)** : diagnostic « le parent ne reçoit pas le rappel du mardi » → cause = **dry-run jamais coupé** depuis `0.3.0` (le scheduler tournait bien, mais le `MailerService` court-circuitait le transport). Correctif ops (pas de code applicatif) : dans `.env.server.enc` (sops, édité en place — plaintext en `/dev/shm/…/.env.server` pour matcher la `creation_rule`, cf. Lot 7 / doc 29), pose de **`NOTIF_EMAIL_DRY_RUN=false`** + **`NOTIF_SMTP_USER=edouard.zemb@gmail.com`** (mot de passe SMTP = `ALERTMANAGER_SMTP_PASSWORD` déjà mappé), **allowlist laissée vide** (cf. anomalie **AN-14** [doc 22](22-registre-anomalies.md) : l'allowlist compare le `to` **entier** → inutilisable dès qu'un foyer a 2 parents). `.enc` re-chiffré versionné (deploy key serveur _read-only_ → commit/push depuis le poste). Recréation : `with-secrets.sh … up -d --no-deps --force-recreate svc-notifications` — ⚠️ **forcer `IMAGE_TAG=<tag prod>` + `compose pull` AVANT le `up`** : `IMAGE_TAG` par défaut (`0.1.0`) fait un **build local** (« manifest unknown ») au lieu de tirer l'image officielle. **Routage multi-parents = déjà en place** (aucun code) : `DestinatairesService.emailsActifs` → `to = emails.join(', ')`. ⚠️ **Risque mail service** : dry-run off arme aussi le mail à la crèche (bouton manuel « Envoyer » sur semaine `VALIDEE_AVEC_MODIFS`) — l'établissement **Crèche L'Accueillette** a une **vraie** adresse `jaudrey@cscpapin.asso.fr` (les autres = placeholders `@example.org`). Reste : test SMTP live + 1er récap multi-parents auto le mardi suivant.
- **Correctif AN-14 + runtime Docker non-root (2026-07-01, PR #128, Lot 0 de l'audit)** : l'allowlist du `MailerService` est désormais vérifiée **par destinataire** (helper exporté `partitionnerParAllowlist`, `libs/nest-commons`) et non plus sur le champ `to` entier — les adresses autorisées partent, les hors-liste sont neutralisées (`warn`) **sans bloquer les autres**, zéro adresse retenue ⇒ dry-run ; une adresse absente de la liste (ex. la vraie crèche `jaudrey@cscpapin.asso.fr`) ne peut **jamais** recevoir, même accompagnée d'une adresse autorisée (testé). `dryRunEffectif` (mail établissement) aligné sur le même helper. **L'allowlist redevient utilisable comme garde-fou multi-parents** (toujours vide en prod à ce jour — à poser au prochain rodage si souhaité). Même PR : **`USER 1000:1000`** au stage runtime du `Dockerfile` (défense dans l'image ; vérifié : aucune écriture fichier au runtime, ports ≥ 3000). ⚠️ **Correction du 2026-08-15 (lot 8 des standards)** : cette entrée disait « en plus du `user: 1000` du compose serveur ». Ce `user:` n'a jamais existé dans `docker-compose.server.yml` — l'image était, et reste, la seule à porter le non-root ; les composes portent depuis le lot 8 le reste de la posture CIS (`no-new-privileges`, `cap_drop`, `read_only`). AN-14 clos au [doc 22](22-registre-anomalies.md) (DDP recalé sur 12 défauts clos).
- **Événement `notifications.SemaineValidee.v1` (outbox activée, 2026-07-05, PR #168)** : jusqu'ici la validation d'une semaine (`ValidationService.valider`) mettait à jour `notification_hebdo` **sans émettre aucun événement** — pas d'historisation, aucun consommateur aval possible (audit, métriques, futur récap établissement automatique). L'**outbox latente** posée au Lot 0 devient **active** : `svc-notifications` devient émetteur. Nouvelle lib de contrat **`libs/contracts/notifications`** (`@creche-planner/contracts-notifications`, `type:contracts`/`context:notifications`, application directe d'[ADR-0004](adr/0004-decentralisation-des-contrats.md)) portant `notifications.SemaineValidee.v1` `{ contratId, semaineIso, statut VALIDEE|VALIDEE_AVEC_MODIFS, deltaModifs? }` + canal AsyncAPI. L'événement est inséré **dans la même transaction** que la transition de statut, publié par l'`OutboxRelay` sur le stream `NOTIFICATIONS` (dédup `id`) — `OutboxModule.forRoot({ source, table })` câblé dans l'`AppModule` (même patron que `svc-foyer`/`svc-planification`). **Émis uniquement à la transition réelle** : l'`UPDATE` porte un garde compare-and-set `WHERE statut = 'A_VALIDER'` + `RETURNING` → une **revalidation idempotente** (état déjà figé) ou une **validation concurrente perdante** n'émet rien et relit l'état figé (réponse HTTP inchangée). **Aucune migration** (table `outbox` présente depuis le Lot 0), **aucune rupture Pact** (spec provider inchangée). Consommateurs aval non encore branchés — ils disposent désormais du contrat.
...
> rattachés au foyer (multi-familles), le parent devenant aussi le **propriétaire naturel** d'un futur
> abonnement web push (hors périmètre). **Décisions de cadrage actées (2026-06-26)** : (B) **identité /
> login** — les parents deviennent des identités authentifiables avec **autorisation par foyer** ; (B1)
> **Cloudflare Access** — on fait confiance à l'e-mail validé par CF (le BFF valide le JWT
> `Cf-Access-Jwt-Assertion`, jamais le header brut), on mappe `email → parent → foyer` ; (b-ii)
> **provisioning admin** (allowlist `ADMIN_EMAILS`) — seul un rôle admin crée les foyers et rattache les
> parents ; **e-mail globalement unique** (= identifiant de login) ; accès notifications→parents =
> **projection NATS** (pas de nouveau Pact). Conception détaillée : `.claude/plans/parents-foyer-modelisation.md`.
> Principe directeur du découpage : **modèle + destinataires d'abord (sans risque), enforcement
> d'autorisation EN DERNIER et derrière un flag** — pour ne jamais verrouiller la prod avant le back-fill des
> e-mails parents. **8 PR, 1 phase = 1 PR.**

- **PR 1 — svc-foyer : modèle `parent` + CRUD + événements + résolution** (✅ livrée, #73) :
  - **contracts-foyer** : `parentIdSchema` brandé (`z.string().uuid().brand<'ParentId'>()`) + événements
    `foyer.ParentAjoute/Modifie/Retire.v1`. `Ajoute`/`Modifie` transportent l'**état complet** du parent
    (`{ foyerId, parentId, email, prenom?, nom?, principal, actif }`) pour que le consommateur projette sans
    relire la source ; `ParentRetire` ne porte que les identités (le retrait est un **soft-delete**).
  - **Schéma drizzle `parent`** (`id` PK, `foyer_id` FK cascade, `prenom`/`nom` NULL, `email` NOT NULL,
    `principal`/`ordre`/`actif`, timestamps) + migration **générée** `0001_parent` (jamais à la main ; table
    neuve → aucun arbitrage drop-vs-rename). Deux index d'unicité : **`UNIQUE` global `lower(email)`**
    (l'e-mail est le futur identifiant de login, option B) et un **index partiel** garantissant **au plus un**
    parent `principal` par foyer.
  - **`FoyerService`** : `ajouterParent`/`listerParents`/`modifierParent`/`retirerParent`, chacune
    **transactionnelle** (écriture + insert outbox dans la **même tx**, sur le modèle d'`ajouterEnfant`) ;
    `foyersParEmail(email)` (résolution identité→foyers, `SELECT DISTINCT` sur les parents **actifs**,
    insensible à la casse). Retrait = soft-delete `actif=false`. Violation d'unicité (e-mail / principal)
    traduite en **409** explicite.
  - **Controller** : `POST|GET|PUT|DELETE /foyers/:id/parents[/:parentId]` (CRUD ; `GET` liste les parents
    actifs) + `GET /foyers?parentEmail=…` (résolution → liste de `foyerId`). DTO/Zod avec validation e-mail
    (`z.email`) ; `PUT` = édition partielle (champs fournis seulement, `null` efface prénom/nom).
  - **Pact provider INCHANGÉ** : aucune nouvelle interaction côté api-gateway en PR1 → les provider states
    parent seront ajoutés avec le consumer en PR2. Tests : service (transactionnalité outbox, 404,
    soft-delete, 409, résolution), DTO (e-mail/défauts/édition partielle), événements. lint/typecheck/test/
    build `svc-foyer` + `contracts-foyer` **verts** ; CI bout-en-bout verte (build-images, smoke/e2e-stack →
    migration `0001_parent` appliquée et stack bootée sur Postgres réel). **Piège reproduit** : `nx build`
...
## 27. Contrat ↔ enfant par référence (`enfantId`) — fin du couplage par prénom libre

**But** — le contrat (`svc-planification`) référençait l'enfant par son **prénom libre**
(`contrat.enfant varchar`) : renommer un enfant (`PUT /foyers/:id/enfants/:enfantId`, svc-foyer)
**orphelinait** ses contrats, deux enfants homonymes étaient indistinguables, et le front faisait un
rapprochement fragile `enfants.find((e) => e.prenom === contrat.enfant)` (`ContratForm`). Désormais le
contrat porte **`enfant_id` (UUID de l'agrégat svc-foyer)** comme **lien de référence**, et `enfant`
(prénom) devient une **dénormalisation d'affichage** rafraîchie par projection NATS.

### 27.1 Architecture livrée

- **Schéma svc-planification** : `contrat.enfant_id uuid` **NULLABLE** (migration `0005`, additive) —
  pas de FK (référence inter-services, comme `foyer_id`). Promotion **NOT NULL différée** après
  back-fill prod, même séquence que `etablissement_id` (P2→P5, migration `0004`). La migration `0005`
  crée aussi `processed_event` (idempotence de consommation, calqué svc-tarification).
- **Projection NATS (nouveau consumer)** : `svc-planification` consomme désormais le stream `FOYER`
  (durable `planification-foyer`, `consumers/{jetstream.consumer,projection.service}.ts` calqués sur
  svc-tarification). Sur **`foyer.EnfantModifie.v1`** : `UPDATE contrat SET enfant = prenom WHERE
enfant_id = :enfantId AND enfant <> :prenom` **+ ré-émission d'un `ContratModifie` par contrat
  touché** (outbox, même transaction) → les read-models aval (`svc-notifications` : templates récap ;
  `svc-tarification` : identité prestations) se rafraîchissent **sans aucun changement de code chez
  eux**. `EnfantAjoute`/`EnfantRetire` : acquittés sans action.
- **Événements** : `ContratCree.v1`/`ContratModifie.v1` gagnent `enfantId` **optionnel/nullable**
  (évolution additive non rupteur, même pattern que `etablissementId` en P2).
- **API** : `enfantId` **requis** (UUID) à la création/modification de contrat (DTO Zod provider +
  validation de forme BFF), exposé `string | null` dans `ContratVue` (null = contrat historique pas
  encore rapproché). Endpoint **chirurgical** `PUT /contrats/:id/enfant` (interne, hors gateway/OpenAPI)
  : ne touche QUE `enfant_id`, n'invalide PAS les plannings, idempotent, émet `ContratModifie` — le
  pendant exact de `PUT /contrats/:id/etablissement` (back-fill P5).
- **Back-fill** : `scripts/backfill-enfants.mjs` (calqué `backfill-etablissements.mjs`) — dry-run par
  défaut, rapprochement **par prénom au sein du foyer** avec **garde d'ambiguïté** (0 ou >1 homonyme →
  signalé, non rattaché), écrit via l'endpoint chirurgical (JAMAIS de SQL brut : l'événement doit
  partir), vérification post-run (reste > 0 ⇒ exit 1).
- **OpenAPI/web** : `ContratVue.enfantId` + `enfantId` requis au POST (types régénérés) ; `ContratForm`
  initialise le select depuis `contrat.enfantId` (**fin du find par prénom**) et envoie `enfantId` +
  `enfant` ; `socleContratDurable` reconduit le lien ; seed e2e passe l'id des enfants créés.
- **Pacts** : interactions contrat (création cantine/ALSH, liste, lecture cœur, modification) portent
  `enfantId` ; provider states seedent `enfant_id` (`ENFANT_SEED_ID`, UUID v4 RFC — Zod 4 strict) ;
  matrice `can-i-deploy` inchangée.

### 27.2 ⚠️ Séquence de déploiement (gestes ordonnés)

1. Train de release : migration `0005` (additive, nullable) + nouveau consumer — **aucun secret, aucune
   variable d'env nouvelle**. Les contrats existants ont `enfant_id NULL` : l'UI fonctionne (le select
   d'édition retombe sur « Sélectionner un enfant » pour eux).
2. **Back-fill prod** (comme P5 établissements) : `node scripts/backfill-enfants.mjs` (dry-run, lire le
   plan) puis `--apply` en ciblant la pile interne (`BACKFILL_FOYER_URL`/`BACKFILL_PLANIFICATION_URL`).
