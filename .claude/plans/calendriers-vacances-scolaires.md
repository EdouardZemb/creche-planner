...
- **D2 — tables (svc-planification)** :
  ...
  - **Chevauchement de périodes — départage par la spécificité, arrêté au lot 1 (revue de la PR #352).** L'import ne publiant que les **vacances**, l'année scolaire saisie d'un bloc les contient : deux périodes couvrent donc le même jour par construction. La **plus courte** l'emporte (ordre total : étendue croissante, puis connaissance la plus récente, puis début le plus tardif) — départager par l'instant de connaissance rendrait le résultat dépendant de l'**ordre de saisie**, et l'import du lot 3 le heurterait de plein fouet (`LE-84`). Le lot 3 n'a donc rien à ordonner : il insère, la résolution tranche.
...
> **LIVRÉ le 2026-08-19** (branche `feat/calendrier-domaine-versionne`) — `libs/shared-kernel/{instant,jours-feries,date-iso}.ts` et `libs/planification/domain/src/lib/calendrier-ouverture.ts`, 100 % de couverture maintenue sur les deux libs, 5 invariants MBT. Consignés : `AM-104`, `LE-82`, `EM-19`.
...
- **L'ancre de connaissance est tranchée ici** (les deux candidats sont en D6 révisée), avec son motif écrit dans le module. C'est la décision la plus lourde du chantier : elle détermine si un mois régénéré pour une autre raison conserve son interprétation.
  → **TRANCHÉ le 2026-08-19 : candidat (a), l'instant de facturation du mois** (`ancreDeConnaissance(maintenant, factureLe?)`, motif complet dans `calendrier-ouverture.ts`). Le motif n'est pas le coût mais la règle : RM-31-03 protège les mois **déjà facturés**, et eux seuls. Le candidat (b) figerait un mois dès la création de son planning — souvent des semaines avant sa facturation — de sorte qu'une correction de calendrier faite en septembre ne s'appliquerait pas à un mois d'octobre saisi en août ; il viderait aussi CA4 de son sens. Règle retenue : **un mois facturé garde l'interprétation qu'il avait à sa facturation ; un mois non facturé suit le calendrier d'aujourd'hui.** Conséquence pour le lot 4 : le chemin de facturation enregistre `factureLe` au premier arrêté et ne le rebouge plus ; tant que la colonne n'existe pas, `factureLe` est `undefined` et le comportement est identique à l'actuel (déployable sans reprise).
...
> pact encadrant une retouche, seed de démo par l'API. Consignés : `AM-106` **clos**, `AM-116`,
> `LE-94`, `LE-95`, `LE-96`.
>
...
- `apps/svc-planification` : **migration `0010`** (⚠️ relevé corrigé le 2026-08-19 : la dernière existante n'est plus `0008_wide_iron_monger.sql` mais `0009_purge_bornes_temporelles.sql`) — tables D2 **historisées selon le lot 1** + colonnes établissement ; `calendrier.service.ts` (CRUD périodes/exceptions/récurrences en **append-only**, lecture résolue, validation zod des jsonb `services`). **Forme imposée par le lot 1** : chaque table porte `connu_depuis timestamptz NOT NULL` et `connu_jusqua timestamptz NULL` (intervalle **semi-ouvert**, `connuJusqua` exclusif — à ne pas aligner sur la sémantique inclusive de `au`) ; les unicités partielles sont exactement les clés de `verifierUniciteOuverte` — `UNIQUE (etablissement_id, jour) WHERE connu_jusqua IS NULL` et `UNIQUE (etablissement_id, regime, jour_semaine) WHERE connu_jusqua IS NULL` ; ⚠️ un index partiel se crée **dans** la transaction du migrator (pas de `CONCURRENTLY`, cf. `AM-62`) ; `calendrier.controller.ts` : `GET /api/etablissements/:id/calendrier?du=&au=&aLaDate=` (jours résolus), `GET/PUT /api/etablissements/:id/calendrier/recurrences`, `POST/DELETE /api/etablissements/:id/calendrier/exceptions`, `POST/PUT/DELETE …/periodes`. Scoping : patron `@ScopeFoyerInterServices({ resoudre: 'etablissement', param: 'id' })` **déjà en place** (`apps/svc-planification/src/etablissement/etablissement.controller.ts:53-71`, résolveur `security/resolveur-foyer.ts`, ressource `'etablissement'`).
...
- ~~`nx <svc>:typecheck` est lib-only (ne typecheck pas les specs) — la CI si ; reproduire avec `tsc --build tsconfig.spec.json` depuis le dossier du service.~~ ⚠️ **Périmé pour `svc-planification`, constaté au lot 2 (`LE-95`)** : son `tsconfig.json` référence `tsconfig.app.json` **et** `tsconfig.spec.json`, et `nx run svc-planification:typecheck` a bien signalé 17 erreurs de specs. Le contournement était du travail en double. Rejouer la commande avant de la contourner — le piège peut rester vrai ailleurs.
