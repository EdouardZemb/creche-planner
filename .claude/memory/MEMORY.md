...
- [Le coût ne ment plus](chantier-cout-ne-ment-plus.md) — **lot 1 MERGÉ le 2026-08-17 (PR #336, `c1086f7`), NON DÉPLOYÉ** : le coût d’un mois qu’aucune version de ressources ne couvre **refuse** (422) au lieu de rendre 202,88 € faux et muet ; **prochain = lot 2** (`AM-58` envois bornés + `AM-57` consentement explicite).
  ⚠️ une garde posée sur « aucune ligne » ne voit rien (l’amont rend toujours une prestation, à quantités nulles), un 4xx déterministe ouvrait le disjoncteur partagé, deux jeux de test ne passaient que tant que la date du jour le voulait ; restent `AM-88` (purge T1) et `AM-90` (mensualité crèche), deux arbitrages PO.
