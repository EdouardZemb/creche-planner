...
- [Le coût ne ment plus](chantier-cout-ne-ment-plus.md) — **lot 1 MERGÉ (`c1086f7`) et lot 2 EN PR, tous deux NON DÉPLOYÉS** : le coût d’un mois non couvert **refuse** (422) ; le récap à la crèche ne part plus ni trop tard ni pour ne rien dire, et le consentement est **écrit** (une ligne supprimée ne réabonne plus).
  ⚠️ une garde sur « aucune ligne » ne voit rien quand l’amont en rend toujours une ; une règle relative à « maintenant » périme tous les jeux figés (`LE-70`, pact compris) ; aucun contrôle n’exécute un back-fill sur des données (`EM-16`) ; restent `AM-88`, `AM-90`, `AM-98`.
