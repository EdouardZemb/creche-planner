...
- [Le coût ne ment plus](chantier-cout-ne-ment-plus.md) — **lots 1 (`c1086f7`) et 2 (`5516e00`) MERGÉS le 2026-08-17, tous deux NON DÉPLOYÉS** : le coût d’un mois non couvert **refuse** (422) ; le récap à la crèche ne part plus ni trop tard ni pour ne rien dire, et le consentement est **écrit** (une ligne supprimée ne réabonne plus). **Prochain = lot 3 « les files qui se taisent »** (`AM-53`, `AM-61`).
  ⚠️ une garde sur « aucune ligne » ne voit rien quand l’amont en rend toujours une ; une règle ou un défaut de lecture retiré périme tous les jeux figés (`LE-70`, `LE-73` — pact compris) ; aucun contrôle n’exécute un back-fill sur des données (`EM-16`) ; restent `AM-88`, `AM-90`, `AM-98`.
