...
- [Calendriers & vacances scolaires (SFD 31)](chantier-calendriers-vacances.md) — 5 lots ; **lot 1 mergé (#352), lot 2 livré le 2026-08-20** : le calendrier existe en base (migration `0010`, append-only) et se lit par un contrat **gelé** dont `aLaDate` est réverbéré ; **lot 3 = prochain**.
  ⚠️ `AM-106` tranché — le régime de fériés est **historisé** (4ᵉ table), pas une colonne d'`etablissement` ; « supprimer » est une **clôture** ; borne de connaissance **exclusive** vs `au` métier **inclusif**.
