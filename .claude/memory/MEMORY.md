...
- [Revue standards industriels 2026-08](revue-standards-2026-08.md) — plan `.claude/plans/plan-standards-industriels.md` ; **lots 0 → 7 mergés** (doc 37, effacement, bornes, portabilité, RFC 9457, validation d'environnement, piste d'audit acteur, sémantique HTTP) et **lot 8 en PR #332** (durcissement CIS des conteneurs, quarantaine npm) ; portes `retentions` / `portabilite` / `problemes` / `environnement` / `acteur` / `conteneurs` / `quarantaine` ; prochain = **lot 9 WCAG 2.2 (`AM-49`)** — `AM-47` (SPF/DKIM/DMARC) reste ⏸ sur décision PO.
