...
- [Calendriers & vacances scolaires (SFD 31)](chantier-calendriers-vacances.md) — 5 lots ; **lot 1 MERGÉ le 2026-08-20 (#352, `df77ebf`), NON DÉPLOYÉ** : domaine calendrier versionné + `joursFeries` ; l'**ancre de connaissance est tranchée** (instant de facturation du mois). **Lot 2 = prochain.**
  ⚠️ Borne de connaissance **exclusive** vs `au` métier **inclusif** ; unicités **partielles** imposées au lot 2 ; migration = **`0010`** ; `regimeFeries` hors axe de connaissance (`AM-106`).
